...
func (c *client) executeStatement(statement string) error {
	if statement == "" {
		return errors.NewTektiteErrorf(errors.StatementError, "statement is empty")
	}
...
func (c *client) RegisterWasmModule(modulePath string) error {
	if !strings.HasSuffix(modulePath, ".wasm") {
		return errors.NewTektiteError(errors.WasmError, "wasm module must have '.wasm' suffix")
	}
	modBytes, err := os.ReadFile(modulePath)
	if err != nil {
		return errors.NewTektiteErrorf(errors.WasmError, "failed to read wasm file '%s: %v", modulePath, err)
	}
	metaFile := modulePath[:len(modulePath)-5] + ".json"
	jsonBytes, err := os.ReadFile(metaFile)
	if err != nil {
		return errors.NewTektiteErrorf(errors.WasmError, "failed to read wasm json file '%s: %v", metaFile, err)
	}
...
func maybeConvertConnectionError(err error) error {
	if err != nil {
		...
		if errors.As(err, &urlErr) && urlErr != nil {
			...
			if errors.As(urlErr.Err, &netErr) && netErr != nil {
				return errors.NewTektiteErrorf(errors.ConnectionError, "connection error: %v", netErr)
			}
		}
		return err
	}
...
func (c *client) extractError(resp *http.Response) error {
	if resp.StatusCode != http.StatusOK {
		...
		if len(bodyString) > 10 && strings.HasPrefix(bodyString, "TEK") {
			...
			msg := bodyString[10:]
			return errors.NewTektiteErrorf(errors.ErrorCode(errorCode), msg)
		}
