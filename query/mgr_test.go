...
func TestQueryVersionsSnapshotIsolation(t *testing.T) {
	...
	writeDataToSlabWithVersion(t, slabID, schema, keyCols, defaultNumPartitions, data2, ctx.st, 13)
	executeQueryFromMgr(t, "test_query1", schema, keyCols, expectedKeyVals, argVals, data, 1, mgr)
...
func writeDataToSlab(t *testing.T, slabID int, schema *evbatch.EventSchema, keyCols []int, numPartitions int, data [][]any,
	st *store.Store) {
	writeDataToSlabWithVersion(t, slabID, schema, keyCols, numPartitions, data, st, 0)
}

func writeDataToSlabWithVersion(t *testing.T, slabID int, schema *evbatch.EventSchema, keyCols []int,
	...
	for _, row := range data {
		...
		prefix := encoding.EncodeEntryPrefix(uint64(slabID), uint64(partID), 16+len(keyBuff))
