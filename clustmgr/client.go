...
func NewClient(keyPrefix string, clusterName string, nodeID int, endpoints []string, leaseTime time.Duration,
	...
	clusterHead := fmt.Sprintf("%s%s/", keyPrefix, clusterName)
	nodesKey := fmt.Sprintf("%s%s", clusterHead, "nodes/")
	thisNodeKey := fmt.Sprintf("%s%d", nodesKey, nodeID)
	clusterStateKey := fmt.Sprintf("%s%s", clusterHead, "cluster_state")
	locksKey := fmt.Sprintf("%s%s", clusterHead, "locks/")
	validGroupsKey := fmt.Sprintf("%s%s", clusterHead, "valid_groups/")
	c := &client{
		nodesKey:            nodesKey,
...
func (c *client) Start() error {
	...
	log.Infof("%s: node %d etcd client starting", c.logScope, c.nodeID)
	...
	log.Infof("%s: node %d etcd client adding node key %s", c.logScope, c.nodeID, c.thisNodeKey)
	...
	log.Infof("%s: node %d etcd client added node key %s", c.logScope, c.nodeID, c.thisNodeKey)
	...
	for _, kv := range getResp.Kvs {
		log.Infof("%s: node %d etcd client got initial node state %s", c.logScope, c.nodeID, string(kv.Key))
...
func (c *client) handleNodesWatchEvents(watchResp clientv3.WatchResponse) {
	c.lock.Lock()
	defer c.lock.Unlock()
	log.Infof("%s: node %d etcd client got node watch event", c.logScope, c.nodeID)

	if c.isStopped() || c.shuttingDown {
		log.Infof("%s: node %d etcd client got node watch event but not processing it as stopped? %t shuttingdown %t", c.logScope, c.nodeID, c.isStopped(), c.shuttingDown)
		return
	}
	for _, event := range watchResp.Events {
		key := string(event.Kv.Key)
		if event.Type == clientv3.EventTypeDelete {
			log.Infof("%s: node %d etcd client handling node delete: %s", c.logScope, c.nodeID, key)
			if err := c.handleNodeDelete(key, event.Kv.ModRevision); err != nil {
				log.Errorf("failed to handle delete %v", err)
			}
		} else {
			log.Infof("%s: node %d etcd client handling node add: %s", c.logScope, c.nodeID, key)
...
func (c *client) SetClusterState(clusterState *ClusterState, nodesState map[int]int64, version int64, maxRevision int64) (bool, error) {
	log.Infof("%s: client node id %d setting cluster state %v", c.logScope, c.nodeID, clusterState)
	ctx, cancel := context.WithTimeout(context.Background(), c.callTimeout)
	defer cancel()
	txn := c.cli.Txn(ctx)
