...
func (sm *ClusteredStateManager) handleActiveNodes(activeNodes map[int]int64, maxRevision int64) {
	sm.lock.Lock()
	defer sm.lock.Unlock()
	log.Infof("%s: node %d etcd clustmgr handleActiveNodes %v", sm.logScope, sm.nodeID, activeNodes)

	if sm.stopped || sm.frozen {
		log.Infof("%s: node %d etcd clustmgr handleActiveNodes not processing stopped %t frozen %t", sm.logScope, sm.nodeID, sm.stopped, sm.frozen)
		return
	}
	if len(activeNodes) == 0 {
		log.Infof("%s: node %d etcd clustmgr handleActiveNodes not processing as no active nodes", sm.logScope, sm.nodeID)
		return
	}
	...
	log.Infof("%s: node %d etcd clustmgr leader node is %d", sm.logScope, sm.nodeID, leaderNode)

	if leaderNode == sm.nodeID {
		// We are the cluster leader

		log.Infof("%s: node %d etcd clustmgr, we are cluster leader creating concurrency session", sm.logScope, sm.nodeID)

		// create a session for distributed locking
		session, err := concurrency.NewSession(sm.client.GetEtcdClient())
		if err != nil {
			log.Errorf("client %d failed to create session %v", sm.nodeID, err)
			return
		}
		...
		defer func() {
			if err := session.Close(); err != nil {
				...
				log.Infof("%s: node %d etcd clustmgr, closed concurrency session", sm.logScope, sm.nodeID)
			}
		}()

		log.Infof("%s: node %d etcd clustmgr, we are cluster leader creating mutex", sm.logScope, sm.nodeID)

		clusterHead := sm.client.GetClusterHead()
		mutex := concurrency.NewMutex(session, fmt.Sprintf("%s/clusterstatelock", clusterHead))

		log.Infof("%s: node %d etcd clustmgr, we are cluster leader created mutex", sm.logScope, sm.nodeID)

		// attempt to acquire the lock
		if err := mutex.Lock(context.Background()); err != nil {
			log.Errorf("failed to get mutex: %v", err)
			return
		}
		...
		defer func() {
			if err := mutex.Unlock(context.Background()); err != nil {
				...
				log.Infof("%s: node %d etcd clustmgr, unlocked mutex", sm.logScope, sm.nodeID)
			}
		}()

		log.Infof("%s: node %d etcd clustmgr, we are cluster leader - got mutex", sm.logScope, sm.nodeID)
		...
		log.Infof("%s: node %d etcd clustmgr, got existing cs: %v ns: %v", sm.logScope, sm.nodeID, cs, ns)
		...
		if cs != nil {
			...
			if csMaxRevision >= maxRevision {
				log.Infof("%s: node: %d ignoring cluster state as already processed by another leader", sm.logScope, sm.nodeID)
				return
			}
		...
		log.Infof("%s: node %d created new clusterstate %v version %d", sm.logScope, sm.nodeID, newState, ver)
