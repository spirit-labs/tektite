...
func TestKafkaTopicNotFound(t *testing.T) {
	...
	err = client.ExecuteStatement(`topic1 :=  (kafka in partitions=10) -> (store stream)`)
	require.NoError(t, err)
	wg.Wait()
	waitForRows(t, "topic1", 1, client, startTime)
}
...
func TestKafkaProduceMultipleTopics(t *testing.T) {
	...
	require.NoError(t, err)
	err = client.ExecuteStatement(`topic3 :=  (kafka in partitions=10) -> (store stream)`)
	require.NoError(t, err)
	...
	waitForRows(t, "topic1", numMsgs, client, startTime)
	waitForRows(t, "topic2", numMsgs, client, startTime)
	waitForRows(t, "topic3", numMsgs, client, startTime)
}
