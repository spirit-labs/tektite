		...
		if gc.startVersionIf(*gc.flexibleRange) {
			...
			for _, field := range taggedFields {
				...
				if gc.startVersionElse() {
					...
					gc.addImport("github.com/spirit-labs/tektite/errors")
				}
				gc.closeVersionIf()
				gc.decIndent()
			}
...
func genReadSimpleField(structName string, field MessageField, gc *genContext) error {
	...
	case "uuid":
		gc.writeF("%s = common.CopyByteSlice(buff[offset: offset + 16])\n", structName)
		gc.write("offset += 16\n")
...
func genReadData(gc *genContext, field *MessageField, receiverName string, lengthVarName string) {
	...
	case "string":
		...
		gc.writeF("%s = &s\n", receiverName)
	case "bytes":
		gc.writeF("%s = common.CopyByteSlice(buff[offset: offset + %s])\n", receiverName, lengthVarName)
		gc.addImport("github.com/spirit-labs/tektite/common")
	case "records":
		gc.writeF("%s = [][]byte{common.CopyByteSlice(buff[offset: offset + %s])}\n", receiverName, lengthVarName)
		gc.addImport("github.com/spirit-labs/tektite/common")
	default:
