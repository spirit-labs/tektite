...
func (m *AddOffsetsToTxnResponse) Read(version int16, buff []byte) (int, error) {
    offset := 0
    ...
    {
        // reading m.ErrorCode: The response error code, or 0 if there was no error.
...
func (m *AddOffsetsToTxnResponse) Write(version int16, buff []byte, tagSizes []int) []byte {
    var tagPos int
    ...
    // writing m.ErrorCode: The response error code, or 0 if there was no error.
...
func (m *AddOffsetsToTxnResponse) CalcSize(version int16, tagSizes []int) (int, []int) {
    size := 0
    ...
    size += 4
    // size for m.ErrorCode: The response error code, or 0 if there was no error.
    size += 2
    numTaggedFields1:= 0
    numTaggedFields1 += 0
    if version >= 3 {
