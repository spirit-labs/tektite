...
func KeyDecodeString(buffer []byte, offset int) (string, int, error) {
	...
	for {
		if len(buffer) < encGroupSize+1 {
			return "", 0, errwrap.New("insufficient bytes to decode value")
		}
		...
		if padCount > encGroupSize {
			return "", 0, errwrap.Errorf("invalid marker byte, group bytes %q", groupBytes)
		}
		...
		if padCount != 0 {
			...
			for _, v := range group[realGroupSize:] {
				if v != padByte {
					return "", 0, errwrap.Errorf("invalid padding byte, group bytes %q", groupBytes)
				}
			}
			break
		}
