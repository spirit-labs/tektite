...
func TestRPCInternalError(t *testing.T) {
	...
	testRPCError(t, err, func(t *testing.T, tektiteError common.TektiteError) {
		t.Helper()
		require.Equal(t, common.InternalError, tektiteError.Code)
	})
}
...
func testRPCError(t *testing.T, respErr error, checkFunc func(*testing.T, common.TektiteError)) {
	t.Helper()
...
func TestRPCConnectionError(t *testing.T) {
	...
	msg = &remotingmsgs.RemotingTestMessage{SomeField: "foxes"}
	r, err = client.SendRPC(msg, server.ListenAddress())
	require.NoError(t, err)
	resp, ok = r.(*remotingmsgs.RemotingTestMessage)
	require.True(t, ok)
...
func TestBroadcastErrorOneServerInternalErrorNoDelays(t *testing.T) {
	err := errwrap.New("spiders")
	testBroadcastErrorOneServer(t, err, 0, 0, func(t *testing.T, tektiteError common.TektiteError) {
		t.Helper()
		require.Equal(t, common.InternalError, tektiteError.Code)
	})
}
...
func TestBroadcastErrorOneServertektiteErrorDelayError(t *testing.T) {
	err := common.NewTektiteError(common.ExecuteQueryError, "unknown query foo")
	testBroadcastErrorOneServer(t, err, 100*time.Millisecond, 0, func(t *testing.T, tektiteError common.TektiteError) {
		t.Helper()
		require.Equal(t, err.Code, tektiteError.Code)
		require.Equal(t, err.Msg, tektiteError.Msg)
	})
...
func TestBroadcastErrorOneServertektiteErrorDelayNonError(t *testing.T) {
	err := common.NewTektiteError(common.ExecuteQueryError, "unknown query foo")
	testBroadcastErrorOneServer(t, err, 0, 100*time.Millisecond, func(t *testing.T, tektiteError common.TektiteError) {
		t.Helper()
		require.Equal(t, err.Code, tektiteError.Code)
		require.Equal(t, err.Msg, tektiteError.Msg)
	})
...
func testBroadcastErrorOneServer(t *testing.T, respErr error, errDelay time.Duration, nonErrDelay time.Duration,
	checkFunc func(*testing.T, common.TektiteError)) {
	t.Helper()
	numServers := 3
	var servers []*server
