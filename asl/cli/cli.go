...
func NewCli(serverAddress string, tlsConfig client.TLSConfig) *Cli {
	return &Cli{
...
func (c *Cli) Start() error {
	...
	var err error
	c.client, err = client.NewClient(c.serverAddress, c.tlsConfig)
	if err != nil {
		return err
	}
	c.started = true
...
func (c *Cli) ExecuteStatement(statement string) (chan string, error) {
	...
	defer c.lock.Unlock()
	if !c.started {
		return nil, common.Error("not started")
	}
...
func (c *Cli) handleSetCommand(statement string) error {
	...
	if len(parts) != 3 {
		return common.Error("Invalid set command. Should be set <prop_name> <prop_value>")
	}
	if propName := parts[1]; propName == maxLineWidthPropName {
		...
		if err != nil || width < minLineWidth || width > maxLineWidth {
			return errwrap.Errorf("Invalid %s value: %s", maxLineWidthPropName, propVal)
		}
		c.lock.Lock()
		defer c.lock.Unlock()
		c.maxLineWidth = width
	} else {
		return errwrap.Errorf("Unknown property: %s", propName)
	}
	return nil
}
...
func (c *Cli) handleRegisterWasm(statement string) error {
	if !strings.HasPrefix(statement, `register_wasm("`) || !strings.HasSuffix(statement, `")`) {
		return errwrap.Errorf(`Invalid register_wasm command. Must be of form 'register_wasm("/path/to/my_module.wasm")'`)
	}
...
func (c *Cli) handleUnregisterWasm(statement string) error {
	if !strings.HasPrefix(statement, `unregister_wasm("`) || !strings.HasSuffix(statement, `")`) {
		return errwrap.Errorf(`Invalid unregister_wasm command. Must be of form 'unregister_wasm("my_module")'`)
	}
...
func (c *Cli) streamToOut(out chan string, ch chan client.StreamChunk, isQuery bool) int {
	rowCount := 0
	first := true
...
func formatLine(res client.QueryResult, rowIndex int, colWidths []int) (string, error) {
	sb := &strings.Builder{}
	sb.WriteString("|")
...
func (c *Cli) checkErrorAndMaybeExit(err error) error {
	...
	return errwrap.Errorf("connection error: %v", err)
}
